...
function render() {
  let filtered = PRODUCTS.filter(p => {
    ...
    const inSearch = p.name.toLowerCase().includes(query) || p.category.toLowerCase().includes(query);
  ...
  grid.innerHTML = shown.length
    ...
    : `<div class="empty-state">No pieces match “${query}” yet. Try another search or <a href="${waLink("Hi Silk N Siren Femme! I'm looking for: ")}" style="color:var(--wine); text-decoration:underline;">ask us on WhatsApp</a>.</div>`;

  if (countLabel) countLabel.textContent = `${filtered.length} piece${filtered.length !== 1 ? "s" : ""}`;
}
...
function applyHashCategory() {
  ...
  if (hash && CATEGORIES.includes(hash)) activeCategory = hash;
}
